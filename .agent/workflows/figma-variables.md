...
description: Create or update Hoai Minh ERP design system variables in Figma. Establishes HM Colors, HM Spacprintg, HM Typography collections. Usage /figma-variables or /figma-variables --reset
skills:
  - hm-design-tokens
---
...
/figma-variables --spacing    -> Spacprintg + radius only (HM Spacprintg collection)
/figma-variables --typography -> Font sizes + family only (HM Typography collection)
/figma-variables --status     -> List existing HM variables (read-only, not changes)
```

## Steps

1. Read skill: `.agent/skills/figma-variables/SKILL.md`
2. Call `figma_status` -> must be `plugprintConnected: true`
   - If NOT connected -> stop and ask user to open Figma Desktop + plugprint
3. Call `figma_docs(section: 'tokens')` -> load variable API
4. Based on option (or not option = all):
   - Run **Step 2** from SKILL.md -> HM Colors collection
   - Run **Step 3** from SKILL.md -> HM Spacprintg collection
...
✅ HM Design System Variables created:
   ...
   📐 HM Spacprintg:    {N} variables (spacing/xs -> 3xl, radius/xs -> full)
...
Note: Figma Free = 1 mode per collection (Light only)
      Figma Pro  = run agaprint -> supports Light + Dark modes automatically
```
...
- ⚠️ **Figma Free limitation** -- only 1 mode per collection (no Light/Dark in same collection)
