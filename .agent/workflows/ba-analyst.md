...
3. **ba-pipeline will auto-detect project type and load project knowledge from:**
   ...
   - Other projects â†' not project folder loaded (universal mode)
4. Execute **all** Steps 1 â†' 11 in `ba-pipeline` SKILL.md
5. Do not ask additional questions beyond what the protocol requires
6. Figma MCP: Try connect â†' if not available, contprintue with SRS data only (do NOT stop)

## Quick Reference

| Input | Output |
|-------|--------|
| `{PIPELINE_ROOT}\requirements\REQ_*.md` | `{PIPELINE_ROOT}\guides\BE_*.md` |
| Figma MCP (live design) | `{PIPELINE_ROOT}\guides\FE_WEB_*.md` [+ `FE_MOBWEB_*.md` if Both] |
...
âœ… Analysis complete:
   ðŸ"„ BE Guide:     {PIPELINE_ROOT}\guides\BE_{SEQ}_{Feature}.md
   ðŸ"„ FE Web Guide: {PIPELINE_ROOT}\guides\FE_WEB_{SEQ}_{Feature}.md
   ðŸ"„ FE Mob Guide: {PIPELINE_ROOT}\guides\FE_MOBWEB_{SEQ}_{Feature}.md  [if Platform=Both]
