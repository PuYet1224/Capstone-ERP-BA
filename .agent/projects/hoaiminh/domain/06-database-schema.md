# Database Schema - Honda HEAD Hoai Minh ERP
...
> **Note on naming inconsistency:** Some tables use `CreateBy`/`CreateTime`, others use `CreatedBy`/`CreatedTime`. NEW tables MUST use `CreatedBy`/`CreatedTime` (past tense).

## Naming Convention for NEW Entities

> When creating a brand-new table, follow these rules strictly.

### Table Name
- Format: `tbl_{ModuleCode}{EntityName}`
- ModuleCode must be one of: SAL, CS, WH, HR, SYS, LS, CRM, MTB, PART
- EntityName in PascalCase (e.g., `tbl_CRMCustomer`, `tbl_SALQuotation`)

### Column Names
| Rule | Example |
|------|---------|
| PK is always `Code` (bigint, identity) | `Code bigint IDENTITY(1,1)` |
| Display ID is `ID` (nvarchar) | `ID nvarchar(30)` |
| HEAD filter is `Head` (bigint, FK) | `Head bigint NOT NULL` |
| Status is `Status` (int, FK to tbl_LSStatus) | `Status int NOT NULL` |
| Sub-type is `TypeData` (int) | `TypeData int NOT NULL` |
| Boolean flags use `Is{Something}` (bit) | `IsActive`, `IsLock` |
| FK uses target entity name (no "ID" suffix) | `OrderMaster` not `OrderMasterId` |
| Audit: `CreatedBy`, `CreatedTime`, `LastModifiedBy`, `LastModifiedTime` | Always include all 4 |
| String fields: specify max length | `nvarchar(200)` not `nvarchar(MAX)` unless truly unbounded |
| Money fields: use `float` (legacy) or `decimal(18,2)` (preferred for new) | Match existing patterns |

### Required Columns for Every New Table
1. `Code` (bigint, PK, identity)
2. `Head` (bigint, FK -> tbl_LSHead) -- HEAD isolation
3. `Status` (int, FK -> tbl_LSStatus) -- status management
4. `TypeData` (int) -- sub-type discriminator
5. `CreatedBy` (nvarchar 100)
6. `CreatedTime` (datetime)
7. `LastModifiedBy` (nvarchar 100)
8. `LastModifiedTime` (datetime)

## Cross-Reference

> For business meaning of ambiguous field names, see `09-field-glossary.md`.
