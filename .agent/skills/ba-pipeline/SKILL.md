---
name: ba-pipeline
description: Principal BA auto-discovers SRS files (7-Pillar IEEE 29148 format), reads live Figma design via MCP bridge, performs deep cross-analysis of requirements + UI, and produces comprehensive BE + FE implementation guides with full traceability. Deep technical contracts â€” API names, DTOs, enums, shared services â€” are resolved before generating guides.
skills:
  - figma-reader
---
...
```
READ:   C:\ai-pipeline\requirements\     (REQ_*.md â€” SRS input)
READ:   Figma MCP (figma_read)           (live designs â€” ONLY source of visual truth)
WRITE:  C:\ai-pipeline\guides\           (BE_*.md + FE_*.md â€” output for dev teams)
```

> ðŸ”´ **NEVER read from `C:\ai-pipeline\designs\` or local PNG files.** Use Figma MCP exclusively.
...
2. Check `C:\ai-pipeline\guides\` â€” find files WITHOUT a matching guide pair
3. Rule:
...
PILLAR 6 â†’ Extract:
  - Â§6.1 Data consumed â†’ read-only fields in FE, no write endpoints needed
...
1. Call `figma_status` â†’ verify Figma Desktop + Plugin connected
2. **If NOT connected:** Note in both guides:
   > "âš ï¸ Figma MCP not connected during guide generation. Screen specs are based on SRS Pillar 7 only. Re-run /ba-analyst after connecting Figma Desktop + plugin for visual validation."
   
   Then continue with SRS data only.

3. **If connected:**

   a. **Infer frame names** from SRS Pillar 7 Â§7.1:
      - Use "Frame / Screen Names" from Â§7.1 Design Reference
      - If blank â†’ infer from feature name (e.g., feature "Receipt" â†’ search Figma for "MTB020", "Receipt", "Phiáº¿u Thu")
...
- SRS conflicts with domain file â†’ **SRS wins** (newer and more specific)
...
- TBD present but non-blocking â†’ Proceed + flag in guide
...
Save to: `C:\ai-pipeline\guides\BE_{SEQ}_{FeatureName}.md`
...
public static class {Feature}Status
{
    public const int {StatusName1} = {N};   // TypeOfStatus={N}, display: "{Display}" â€” [SRS Â§6.4]
...
Step 3: Query tbl_{PrimaryTable}
        ...
        â†’ .AsNoTracking()                                    â† read-only â†’ performance
...
Step 2: if Code == 0 â†’ CREATE branch:
        â†’ Verify no duplicate {uniqueField} (if BR-xx requires uniqueness)
...
| LABEL MISMATCH | {button} | SRS: "{A}" / Figma: "{B}" | SRS wins â†’ implement as "{A}" |
...
Save to: `C:\ai-pipeline\guides\FE_WEB_{SEQ}_{FeatureName}.md`
...
Save to: `C:\ai-pipeline\guides\FE_MOBWEB_{SEQ}_{FeatureName}.md`
...
---
## 1. Overview
## 2. Service Files (same {module}-api.service.ts â€” reuse, no changes needed if already created for web)
...
ðŸ“¦ Guides created:
   â†’ C:\ai-pipeline\guides\BE_{SEQ}_{Name}.md        ({N} lines)
   â†’ C:\ai-pipeline\guides\FE_WEB_{SEQ}_{Name}.md    ({N} lines)  [if Web/Both]
   â†’ C:\ai-pipeline\guides\FE_MOBWEB_{SEQ}_{Name}.md    ({N} lines)  [if Mobile/Both]
...
ðŸ”— Next steps:
   ...
   PM/BA    â†’ resolve TBD items before sprint start
```

---

## 4. Quality Gates â€” MANDATORY Before Saving Any Guide
...
[ ] Static service URLs correct (POST only â€” no GET/PUT/DELETE)
...
> If information is genuinely unknown â†’ write "âš ï¸ TBD: {specific question}" â€” NOT a generic placeholder.
